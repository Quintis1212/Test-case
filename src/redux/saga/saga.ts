...
mock.onGet("/transactions").reply(200, {
  users: ["TransactionId,Status,Type,ClientName,Amount",
...
let callAPI: any = async () => {
  return await axios.get("/transactions").then(function (response: responseType) {
...
export function* fetchDataSaga() {
  try {
    let result: responseType = yield call(() =>
      callAPI()
    );
